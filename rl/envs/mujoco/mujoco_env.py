...
class MujocoEnv(BaseMujocoEnv):
    ...
    def _get_viewer(
        ...
        "rl.envs.mujoco.mujoco_rendering.Viewer",
        "rl.envs.mujoco.mujoco_rendering.RenderContextOffscreen",
    ]:
        self.viewer = self._viewers.get(mode)
        if self.viewer is None:
            if mode == "human":
                from rl.envs.mujoco.mujoco_rendering import Viewer

                self.viewer = Viewer(self.model, self.data)
            elif mode in {"rgb_array", "depth_array"}:
                from rl.envs.mujoco.mujoco_rendering import RenderContextOffscreen
